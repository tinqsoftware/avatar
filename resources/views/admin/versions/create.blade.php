...
<form class="card form-grid" method="POST" action="{{ route('admin.versions.store', $avatar) }}">@csrf
    ...
    <label>JSON del árbol<textarea name="tree_json" rows="22" required spellcheck="false">{{ old('tree_json') }}</textarea><small>La validación exige greeting, fallback, connectors y temas con summary/detail/next.</small></label>
